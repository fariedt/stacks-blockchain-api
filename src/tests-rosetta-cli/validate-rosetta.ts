...
const recipients = [recipientAdd1, recipientAdd2, recipientAdd3, recipientAdd4,
    recipientAdd5, recipientAdd6, recipientAdd7, recipientAdd8, recipientAdd9,
    recipientAdd10, recipientAdd11, recipientAdd12, recipientAdd13, recipientAdd14,
    recipientAdd15, recipientAdd16, recipientAdd17, recipientAdd18, recipientAdd19, recipientAdd20]
...
describe('Rosetta API', () => {
    ...
    beforeAll(async () => {
        ...
        for (const sender of senders) {
            const response = await deployContract(sender.privateKey, "src/tests-rosetta-cli/contracts/hello-world.clar", api)
...
async function callContractFunction(api: ApiServer, senderPk: string, contractId: string, functionName: string, ...functionArgs: string[]) {
    await waitForBlock(api);
...
async function transferStx(recipientAddr: string, amount: number, senderPk: string, api: ApiServer) {
    await waitForBlock(api);
...
async function sendCoreTx(serializedTx: Buffer, api: ApiServer, type: String): Promise<{ txId: string }> {
...
const isContainerRunning = async (name: string): Promise<boolean> => new Promise((resolve, reject): void => {
    docker.listContainers((err: any, containers: any): void => {
        ...
        const running = (containers || []).filter((container: any): boolean => container.Names.includes(name));
